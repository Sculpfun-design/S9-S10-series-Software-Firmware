...
void mks_draw_power(void) {
	...
	if(sys.spindle_speed == 0) mks_grbl.power_persen = P_0_PERSEN;
...
void draw_power_click(bool status) {
	lv_obj_set_click(power_page.Back, status);
	lv_obj_set_click(power_page.pwr_high, status);
	lv_obj_set_click(power_page.pwr_low , status);
	lv_obj_set_click(power_page.pwr_off , status);
} 