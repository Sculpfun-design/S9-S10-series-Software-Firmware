...
#define FRAME_TASK_PRO                  3          
#define FRAME_TASK_CORE                 1

#define TEST_TASK_STACK                4096
#define TEST_TASK_PRO                  1          
#define TEST_TASK_CORE                 1
...
static void mks_page_data_updata(void) { 
    ...
    if(mks_grbl.is_test_mode == true) {
        if((count_updata == 20) || (count_updata > 20) ) { 
            draw_testing();
            count_updata = 0;
        }
    }
...
//             // 更新完成弹窗
//             mks_draw_common_pupup_info((char *)mc_language.tis_info, "Update succeed", "Please restart!");
...
void test_task(void *parg) {
    ...
    uint32_t tim_coumt = 0;
    // grbl_send(CLIENT_SERIAL, "enter Test task\n");
    while(1) {

        if((tim_coumt == 20) || (tim_coumt > 20) ) { 
            draw_testing();
            tim_coumt = 0;
        }
        tim_coumt++;
        vTaskDelay(5); // base on 5ms to run test task; 
    }   
}

IRAM_ATTR void test_task_init(void ) {
    xTaskCreatePinnedToCore(test_task,             // task
                            "test task",           // name for task
                            FRAME_TASK_STACK,       // size of task stack
                            NULL,                   // parameters
                            FRAME_TASK_PRO,         // priority
                            // nullptr,
