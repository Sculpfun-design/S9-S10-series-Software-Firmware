...
#define DEBUG_ON
void debug_send(const char* text) {
#ifdef DEBUG_ON
    grbl_send(CLIENT_SERIAL, text);
#endif
}

void mks_grbl_parg_init(void) {

    if(language_select->get() == 0) {
        mks_grbl.language = SimpleChinese;
    }else if(language_select->get() == 1) {
        mks_grbl.language = English;
    }else if(language_select->get() == 2) {
    ...
    mks_ui_page.wait_count = 0;                                 
    mks_grbl.carve_times = 0;                                   // 雕刻次数

    mks_grbl.is_reading_file = false;                           // default false
