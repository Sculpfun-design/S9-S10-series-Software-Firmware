...
void mks_updata_init(void) {
    ...
    if(is_exit_file == true) {
        ...
        debug_send("[Info]exit update file\n");
    }else {
        mks_updata.updata_flag = UD_NO_FILE;   // exit cfg updata file
        debug_send("[Info]no exit update file\n");
    }
}
...
void mks_draw_updata(void) {
    ...
    updata_page.label_updata_title = label_for_screen(mks_global.mks_src, updata_page.label_updata_title, 0, -80, "Updating...");
